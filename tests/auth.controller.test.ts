...
const mockRawToken =
  'aaabbbcccddd00112233445566778899aabbccddeeff00112233445566778899'
...
vi.mock('../src/services/otp.service', async () => {
  const actual = await vi.importActual<
    typeof import('../src/services/otp.service')
  >('../src/services/otp.service')
...
describe('AuthController', () => {
  ...
  describe('login', () => {
    ...
    it('should return 403 with scheduledFor for accounts pending deletion', async () => {
      ...
      ;(bcrypt.compare as any).mockResolvedValue(true)
      ;(prisma.accountDeletionRequest.findFirst as any).mockResolvedValue({
        scheduledFor,
      })
  ...
  describe('logout', () => {
    it('revokes the session identified by the refresh token', async () => {
      ...
      vi.mocked(refreshTokenService.revokeByRefreshToken).mockResolvedValue({
        revokedCount: 1,
      })
  ...
  describe('logoutAll', () => {
    it('revokes all sessions for the user identified by the refresh token', async () => {
      ...
      vi.mocked(refreshTokenService.revokeAllByRefreshToken).mockResolvedValue({
        revokedCount: 3,
      })
  ...
  describe('verifyOtp', () => {
    it('should return 400 for an invalid/expired code', async () => {
      ...
      ;(otpService.verifyChallenge as any).mockResolvedValue({
        ok: false,
        reason: 'mismatch',
      })
