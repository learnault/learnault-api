...
describe('OpenAPI document', () => {
  ...
  it('keeps private account fields out of the documented public profile', () => {
    ...
    for (const leak of ['email', 'password', 'walletAddress', 'isVerified', 'phoneVerifiedAt', 'status']) {
  ...
  it('marks the public profile read as unauthenticated and the owner reads as bearer-authenticated', () => {
    expect((spec.paths['/users/{id}'].get as { security: unknown[] }).security).toEqual([])
    expect((spec.paths['/users/me'].get as { security: unknown[] }).security).toEqual([
      { bearerAuth: [] },
    ])
    expect((spec.paths['/users/me'].patch as { security: unknown[] }).security).toEqual([
      { bearerAuth: [] },
    ])
  })
  ...
  it('closes the profile update body so undocumented fields cannot be sent', () => {
    ...
    for (const forbidden of ['status', 'isVerified', 'role', 'userId', 'id', 'password', 'email']) {
      expect(updateInput.properties).not.toHaveProperty(forbidden)
    }
  })

  it('documents the conflict response on the wallet update', () => {
    const responses = (spec.paths['/users/wallet'].patch as { responses: Record<string, unknown> })
      .responses
