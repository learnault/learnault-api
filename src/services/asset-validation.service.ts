...
} from './storage/in-memory-storage'
...
export function validateAvatarBytes(
  ...
  if (actualSize < AVATAR_MIN_BYTES) {
    return {
      ...
      detectedMime: null,
      dimensions: null,
      error: 'File too small (minimum 1 KB)',
    }
  }
  ...
  // ── Allowlist check ───────────────────────────────────────────
  if (
    !(AVATAR_ALLOWED_MIME_TYPES as readonly string[]).includes(detectedMime)
  ) {
    return {
      ok: false,
      detectedMime,
      dimensions: null,
      error: `Unsupported image type: ${detectedMime}`,
    }
  }
  ...
  const declaredNormalised = declaredContentType
    .split(';')[0]
    .trim()
    .toLowerCase()
