...
export const resendAccountCounts = new Map<
  string,
  { count: number; resetAt: number }
>()
export const resetPasswordCooldowns = new Map<string, number>()
export const resetPasswordAccountCounts = new Map<
  string,
  { count: number; resetAt: number }
>()
export const otpPhoneCounts = new Map<
  string,
  { count: number; resetAt: number }
>()
export const otpDeviceCounts = new Map<
  string,
  { count: number; resetAt: number }
>()
...
export class AuthController {
  ...
  async register(req: Request, res: Response): Promise<void> {
    try {
      ...
      emailService
        .queueEmail(user.id, email, subject, body)
        .catch((err) =>
    ...
    try {
      ...
      switch (result.kind) {
        ...
        case 'revoked':
          res
            .status(401)
            .json({ error: 'Refresh token revoked', code: 'REFRESH_REVOKED' })
        ...
        case 'invalid':
          res
            .status(401)
            .json({ error: 'Invalid refresh token', code: 'REFRESH_INVALID' })
  ...
  async logout(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await refreshTokenService.revokeByRefreshToken(
        refreshToken,
        this.clientContext(req),
      )
  ...
  async logoutAll(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await refreshTokenService.revokeAllByRefreshToken(
        refreshToken,
        this.clientContext(req),
      )
  ...
  async forgotPassword(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        this.isRateLimited(`reset:user:${user.id}`, RESET_PASSWORD_COOLDOWN_MS)
      ) {
        res
          .status(429)
          .json({ error: 'Too many requests. Please try again later.' })
  ...
  async requestOtp(req: Request, res: Response): Promise<void> {
    try {
      ...
      const purpose: OtpPurpose = authenticatedUserId
        ? 'PHONE_VERIFICATION'
        : 'LOGIN'

      if (
        this.isRateLimited(
          `otp:phone:${purpose}:${normalizedPhone}`,
          OTP_PHONE_COOLDOWN_MS,
        ) ||
  ...
  private async getAccountStatusError(user: {
    id: string
    status: string
  ...
  private recordAccountRequest(userId: string): void {
    ...
    if (!record || record.resetAt < now) {
      resendAccountCounts.set(userId, {
        count: 1,
        resetAt: now + RESEND_ACCOUNT_WINDOW_MS,
      })
    } else {
      record.count++
    }
  ...
  private recordResetPasswordAccountRequest(userId: string): void {
    ...
    if (!record || record.resetAt < now) {
      resetPasswordAccountCounts.set(userId, {
        count: 1,
        resetAt: now + RESET_PASSWORD_ACCOUNT_WINDOW_MS,
      })
    } else {
      record.count++
    }
  ...
  private recordDeviceOtpRequest(deviceId: string): void {
    ...
    if (!record || record.resetAt < now) {
      otpDeviceCounts.set(deviceId, {
        count: 1,
        resetAt: now + OTP_DEVICE_WINDOW_MS,
      })
    } else {
      record.count++
    }
