...
export class RewardController {
  ...
  getHistory = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    if (req.query.type) {
      const validTypes = ['module_reward', 'streak_bonus', 'referral_reward', 'withdrawal'];
    ...
    if (req.query.fromDate) {
      ...
      if (isNaN(date.getTime())) {
        throw new BadRequestError('Invalid fromDate format. Use ISO 8601 format');
      }
    ...
    if (req.query.toDate) {
      ...
      if (isNaN(date.getTime())) {
        throw new BadRequestError('Invalid toDate format. Use ISO 8601 format');
      }
      filters.toDate = date;
    }
