...
function buildDeletionRequestedEmail(
  username: string,
  scheduledFor: Date,
...
function buildDeletionCancelledEmail(username: string): {
  subject: string
  body: string
} {
...
export class AccountController {
  ...
  async requestExport(req: Request, res: Response): Promise<void> {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during export request' })
    }
  }
  ...
  async getExportStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!params.success) {
        res
          .status(400)
          .json({ error: 'Validation failed', details: params.error.format() })

        return
      }

      const request = await dataExportService.getExportStatus(
        req.user!.id,
        params.data.id,
      )
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during export status lookup' })
    }
  }
  ...
  async downloadExport(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        ...
        res
          .status(409)
          .json({ error: 'Export is not ready yet', status: request.status })
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during export download' })
    }
  }
  ...
  async deactivate(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (result.kind === 'conflict') {
        res.status(409).json({
          error:
            result.status === AccountStatus.PENDING_DELETION
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during deactivation' })
    }
  }
  ...
  async reactivate(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (user.status === AccountStatus.PENDING_DELETION) {
        res.status(409).json({
          error:
            'Account is scheduled for deletion. Cancel the deletion request to restore access.',
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during reactivation' })
    }
  }
  ...
  async requestDeletion(req: Request, res: Response): Promise<void> {
    try {
      ...
      const email = buildDeletionRequestedEmail(
        user.username,
        result.request.scheduledFor,
      )
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during deletion request' })
    }
  }
  ...
  async getDeletionStatus(req: Request, res: Response): Promise<void> {
    try {
      const request = await accountLifecycleService.getLatestDeletionRequest(
        req.user!.id,
      )
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during deletion status lookup' })
    }
  }
  ...
  async cancelDeletion(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (result.kind === 'finalized') {
        res.status(410).json({
          error: 'Deletion has already been finalized and cannot be cancelled',
        })
      ...
      res
        .status(500)
        .json({ error: 'Internal server error during deletion cancellation' })
    }
  }
  ...
  private async stepUp(
    ...
    if (!isMatch) {
      ...
      res
        .status(401)
        .json({ error: 'Invalid password', code: 'STEP_UP_FAILED' })
